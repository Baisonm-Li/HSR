...
if __name__ == "__main__":
    """
    PaviaU 数据集的处理方法和Chikusei一样
    """
    loaded_data = scio.loadmat('/wxw/lbs/SR/datasets/PaviaU.mat')
    print(loaded_data.keys())
    stride = 16
    window_size = 128
    tar_size = 64
    mat_data = np.array(loaded_data['paviaU']).transpose(2,0,1)
    ...
    for i in range(0,window_size-tar_size,stride):
        for j in range(0,window_size-tar_size,stride):
            ...
            test_list.append((GT,LRHSI,RGB))
    
    print('==切割训练数据==')
    train_region = mat_data
    train_region[:,h // 2 - window_size // 2:h // 2 + window_size // 2, 
                   w // 2 - window_size // 2:w // 2 + window_size // 2] = torch.zeros((window_size,window_size))
    
    plt.imshow(train_region[:3,:,:].transpose(1,2,0))
    plt.savefig('./paviaU.png')
    # 切割训练数据
    for i in range(0,h-tar_size,stride):
        for j in range(0,w-tar_size,stride):
            if is_overlap(i,j,h,w,window_size=128):
                continue
            else:
                ...
                LRHSI = down_sample(GT)
                RGB = get_RGB(GT,RGB_indexs)
                train_list.append((GT,LRHSI,RGB))   
    train_dict = {
    ...
    np.savez('/wxw/lbs/SR/HSR/datasets/PaviaU_train.npz',**train_dict)
    np.savez('/wxw/lbs/SR/HSR/datasets/PaviaU_test.npz',**test_dict)
    print('==数据保存完毕==')
